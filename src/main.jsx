...
function App() {
  ...
  const deleteScript = async id => { try { if (!confirm('Delete this script?')) return; await request(`/scripts/${id}`, { method: 'DELETE' }); await loadScripts(); } catch (err) { setMessage(err.message); } };
  const logout = () => { localStorage.removeItem('token'); localStorage.removeItem('user'); setToken(null); setUser(null); setScripts([]); };
